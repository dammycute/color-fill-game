...
export const LEVELS = [
  { grid: 6, maxMoves: 9, colors: 4 },
  { grid: 7, maxMoves: 11, colors: 4 },
  { grid: 8, maxMoves: 13, colors: 5 },
  { grid: 9, maxMoves: 16, colors: 5 },
  { grid: 10, maxMoves: 19, colors: 6 },
  { grid: 11, maxMoves: 21, colors: 6 },
  { grid: 12, maxMoves: 24, colors: 6 },
  { grid: 13, maxMoves: 27, colors: 7 },
  { grid: 14, maxMoves: 30, colors: 7 },
  { grid: 15, maxMoves: 33, colors: 7 },
...
export function generateGrid(size, numColors, seed) {
  ...
  for (let r = 0; r < size; r++) {
    for (let c = 0; c < size; c++) {
      let color
      // 15% chance to inherit from neighbor for slightly easier solving (clumping)
      if (r > 0 && rng() < 0.15) {
        color = grid[r - 1][c]
      } else if (c > 0 && rng() < 0.15) {
        color = grid[r][c - 1]
      } else {
